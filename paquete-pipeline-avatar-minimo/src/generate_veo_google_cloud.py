...
def build_prompt(line):
    if line:
        performance = "realistic lip sync,"
        speech = "She says exactly in Argentine Spanish: " f'\"{line}\"'
    else:
        performance = "closed relaxed lips, no lip sync, no mouth flapping,"
        speech = (
            "She does not speak and makes no talking mouth shapes. "
            "She breathes naturally, smiles softly, and waits attentively with tiny idle gestures."
        )
    return (
        ...
        f"subtle head movement, {performance} natural human eyes. "
...
def build_request(prompt, reference_image, output_gcs_uri=None):
    ...
    parameters = {
        ...
        "negativePrompt": "black bars, letterbox, border, dark panel, black background, room background, restaurant background, shadows, gradients, text, subtitles, logos, props, red eyes, glowing eyes, neon eyes, eye color change, open mouth while silent, talking while silent",
    }
    if output_gcs_uri:
