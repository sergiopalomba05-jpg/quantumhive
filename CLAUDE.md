...
11. **Consultá el grafo ANTES de releer el repo.** Hay un grafo de conocimiento en `graphify-out/` (local, gitignorado). Para "¿dónde está X?", arquitectura, o "¿cómo se conecta Y con Z?": preguntale al grafo (`graphify query "..."`, `graphify explain "Nodo"`, o `graphify-out/graph.json`) ANTES de abrir archivos sueltos. Releer todo el repo es el último recurso, no el primero. **Mantenelo vivo:** el hook post-commit lo actualiza en cada commit (código); tras cambios grandes corré `graphify update .` (estructura, sin LLM) y, con crédito LLM, `graphify label .` (nombres de comunidades). Si no existe `graphify-out/`, regeneralo con `/graphify`.
...
14. **PROHIBIDO tomar capturas de pantalla, hacer pruebas en el preview o levantar servidores durante la implementación.** Cada screenshot/preview consume tokens caros sin valor real. El flujo es: aplicar cambios → `py_compile` → commit → push → Sergio prueba en el Space. Las capturas de pantalla quedan eliminadas del flujo de trabajo.
13. **Todo lo visual tiene que ser responsive — probá en varias pantallas ANTES de entregar.** Los productos son móviles (cartas QR, webs, apps): cada cambio de UI debe adaptarse a celulares de distinto **alto y ancho**, no solo al de Sergio. Nada de posiciones fijas que se superpongan: usá flujos flexibles, `clamp()`/`vh`/`vw` y media queries. Verificá el cambio en al menos 2-3 tamaños (ej. 360×560, 360×640, 390×844) y que no se tape ni se corte nada, antes de decir que está listo. (Pasó real: la portada se veía bien en un celu y tapaba todo en otro.)
