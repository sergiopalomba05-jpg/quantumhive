...
- Las keys (`TELEGRAM_TOKEN`, `ANTHROPIC_API_KEY`, `GROQ_API_KEY`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, `HF_TOKEN`, `SUPABASE`, `AWS`) van **solo** en:
  1. Env vars de **HuggingFace Spaces** / **Railway** (producción).
...
**Infraestructura:** HuggingFace Spaces · **Railway** (n8n + servicios, reemplaza a Oracle descartado) · GitHub · Supabase + SQLite.
...
- **Mac 2015 + Windows (Boot Camp):** nada pesado en local.
  ...
  - ✅ Todo lo pesado por **API Cloud**. Procesamiento masivo en Railway o con créditos cloud.
