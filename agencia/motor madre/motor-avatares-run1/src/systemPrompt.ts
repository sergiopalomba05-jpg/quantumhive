...
export function buildSystemPrompt(): string {
  ...
  const assistant = menuData.assistant;
  ...
  lines.push(`Sos la mesera de ${restaurant.name || 'el restaurante'}. Trabajás acá hace años, conocés la carta de memoria y sabés leer a cada cliente.
...
Tu carta completa con todos los precios está abajo (para cuando te los pidan).`);
  ...
  const personalidad = assistant.personality || "";
  if (personalidad) {
    lines.push("TONO PROPIO DE ESTE RESTAURANTE (mantené todo lo de arriba, pero adaptá tu registro a esto):");
    lines.push(personalidad);
    lines.push("");
  }

  // Cart logic
  lines.push(`ARMAR Y MODIFICAR EL PEDIDO — MUY IMPORTANTE:
...
CARTAS DE PLATOS (#CARDS#) — línea técnica invisible:
Cuando hables sobre uno, dos o tres platos ESPECÍFICOS de la carta (para recomendarlos o describirlos), podés mostrar sus cartas (tarjetas visuales con foto y precio) agregando la siguiente línea al final:
#CARDS# ["ID_DEL_PLATO_1", "ID_DEL_PLATO_2"]
- Sólo incluí los IDs exactos de los ítems de la carta (como figuran en la lista abajo).
- Nunca lo leas en voz alta.
- Agregalo siempre DESPUÉS de #PEDIDO# y antes de #CHIPS# (si los hay).
...
quiere cerrar, no en cada respuesta.`);
  ...
  lines.push(`CERRAR vs AGREGAR (clave): si el cliente dice "así está bien", "listo", "nada más", "eso es todo" o toca
...
directo. NUNCA mezcles con alcohol y sin alcohol en la misma tanda.`);
  ...
  lines.push(`PREFERENCIAS DEL CLIENTE (#NOTA#) — otra línea técnica invisible:
...
más abajo. Si no sabés algo del cliente, no lo adivines.`);
  ...
  if (rules.sharing_surcharge) {
    const ss = rules.sharing_surcharge;
    lines.push(`• Adicional por plato compartido: ${symbol}${ss.amount} (${ss.note}). Exception: ${ss.bar_exception}`);
  }
  if (rules.bread) {
    lines.push(`• Pan: ${rules.bread}`);
  }
  if (rules.side_salad_addon) {
    const sa = rules.side_salad_addon;
    lines.push(`• Ensalada acompañamiento: ${symbol}${sa.price} (${sa.note}). Opciones: ${sa.options.join(", ")}`);
  }
  if (rules.single_dish_surcharge) {
    const sds = rules.single_dish_surcharge;
    lines.push(`• Adicional por plato individual: ${symbol}${sds.amount} (${sds.note})`);
  }
  if (rules.kids_menu) {
    const km = rules.kids_menu;
    lines.push(`• Menú infantil: menores de ${km.age_limit} años (${km.note})`);
  }
  if (rules.wine_copa_volume) {
    const wcv = rules.wine_copa_volume;
    lines.push(`• Medidas de copas: Vinos ${wcv.vinos}, Espumantes ${wcv.champagne_y_espumantes}`);
  }
  if (rules.food_safety_note) {
    lines.push(`• Seguridad alimentaria: ${rules.food_safety_note}`);
  }
  if (rules.happy_hour) {
  ...
  for (const section of menuData.menu) {
    ...
    for (const it of section.items) {
      const priceStr = it.price ? `${symbol}${it.price}` : "(precio a confirmar)";
      lines.push(`  · ${it.name} — ${priceStr} ${it.description ? `(${it.description})` : ""}`);
    }
  }

  // Drinks section
  lines.push("\n--- BEBIDAS ---");
  for (const section of menuData.drinks) {
    lines.push(`\n[${section.name}]`);
    if (section.items) {
      for (const it of section.items) {
        const prices: string[] = [];
        if (it.price_copa != null) prices.push(`copa ${symbol}${it.price_copa}`);
        if (it.price_botella != null) prices.push(`botella ${symbol}${it.price_botella}`);
        if (it.price != null) prices.push(`${symbol}${it.price}`);
        const priceStr = prices.join(" / ") || "(precio a confirmar)";
        lines.push(`  · ${it.name} ${it.bodega ? `[${it.bodega}] ` : ""}— ${priceStr}`);
      }
    }
  ...
  lines.push("• Acordate: sos la mesera. Conversás como tal. Si la pregunta es chiquita, respondés corto; si te piden recomendación, te jugás y describís con apetito.");
  lines.push("• IDIOMA: TODO en español rioplatense argentino. NI UNA palabra en portugués (ni 'você', 'obrigada', 'comida deliciosa' a la portuguesa, nada). Si algo te sale en portugués, corregilo a castellano ANTES de mandar la respuesta.");
  lines.push("• El TOTAL del pedido NO lo decís ni lo sumás vos (te equivocás): la app lo muestra exacto en pantalla.");
