...
import { VirtualWaitress } from "./components/VirtualWaitress";
import { OrderSheet } from "./components/OrderSheet";
import { RatingModal } from "./components/RatingModal";
import SolScene from "./components/SolScene";
...
export default function App() {
  ...
  const playGuidedFlow = async (flowKey: string) => {
    ...
    setWaitressState("speaking");
    setIsWaitressOpen(false); // Close drawer to show scrolling and spotlighting!
    setSpotlightDishId(null);
    setCurrentSubtitle("");
  ...
  const handleChipClick = (chip: string) => {
    // If it's a default guided flow chip, handle natively
    if (chip.startsWith("🔥") || chip.startsWith("🍢") || chip.startsWith("🍷") || chip.startsWith("🍰")) {
      playGuidedFlow(chip);
    } else {
      // Check if it matches a dish to trigger scroll, tab update, and spotlight highlight
      const matched = findMenuItemByString(chip);
      if (matched) {
        setSpotlightDishId(matched.item.id);
        setActiveTab(matched.categoryId);
        setIsWaitressOpen(false); // Close the waitress panel so the background scroll is fully visible
      ...
      // Send text message directly to virtual waitress
      handleSendMessage(chip, !isVoiceMuted);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0E0A07] text-[#F4ECD8] selection:bg-accent selection:text-[#F4ECD8] relative overflow-x-hidden">
      ...
      <div className="fixed bottom-6 inset-x-0 h-16 pointer-events-none flex items-center justify-between px-4 sm:px-12 z-30">
        ...
        {/* Central Floating Mic Cosmic Orbe */}
        <div className="relative w-20 h-20 -translate-y-4 pointer-events-auto flex items-center justify-center">
          <motion.button
            ...
            onClick={() => {
              // Open Virtual Waitress drawer
              setIsWaitressOpen(true);
            }}
            animate={{
              rotateX: orbTilt.y,
              rotateY: orbTilt.x,
              boxShadow: waitressState === "listening" 
            ...
            className="w-16 h-16 rounded-full relative z-10 flex items-center justify-center cursor-pointer overflow-hidden border border-gold/40 bg-[#181210]"
          >
            {/* Visual breathing layers (Target child 1 for selector!) */}
            <motion.div
              animate={{
                scale: waitressState === "listening" ? [1, 1.15, 1] : waitressState === "speaking" ? [1, 1.08, 1] : [1, 1.03, 1]
              }}
              transition={{
                duration: waitressState === "listening" ? 1.2 : waitressState === "speaking" ? 0.8 : 4,
                repeat: Infinity,
                ease: "easeInOut"
              }}
              style={{
            ...
            >
              <Suspense fallback={
                ...
                <SolScene audioRef={audioRef} waitressState={waitressState} />
              </Suspense>
              <div className={`absolute inset-0 transition-colors duration-300 ${
                waitressState === "listening" ? "bg-red-500/20" :
                waitressState === "thinking" ? "bg-gold/25" :
                waitressState === "speaking" ? "bg-emerald-500/15" :
              ...
              {waitressState !== "idle" && (
                <motion.div
                  ...
                  className={`absolute inset-0 rounded-full border pointer-events-none ${
                    waitressState === "listening" ? "border-red-500/40" :
                    waitressState === "thinking" ? "border-gold/40" :
            ...
            {/* Mic Icon (Target child 2 for selector!) */}
            <Mic 
              className={`w-5 h-5 text-white relative z-20 drop-shadow-[0_3px_6px_rgba(0,0,0,0.9)] transition-all duration-300 ${
                waitressState === "speaking" ? "scale-0 opacity-0" : "scale-100 opacity-100"
              }`} 
              style={{
            ...
            {/* Simulated soundwave voice frequency equalizer overlay when speaking */}
            {waitressState === "speaking" && (
              <div className="absolute inset-x-0 bottom-0 top-0 flex items-center justify-center gap-[3px] z-20 pointer-events-none px-2 bg-black/60 backdrop-blur-[2px] transition-all duration-300">
                {[
                  { d: 0.5, h: ["4px", "20px", "4px"] },
                  { d: 0.7, h: ["6px", "28px", "6px"] },
                  { d: 0.4, h: ["8px", "32px", "8px"] },
                  { d: 0.6, h: ["10px", "24px", "10px"] },
                  { d: 0.5, h: ["5px", "16px", "5px"] },
                ].map((bar, i) => (
                  <motion.div
                    key={i}
          ...
          {/* Miniature status label above Orbe */}
          <div className="absolute top-[-26px] left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-full border border-gold/20 bg-black/90 backdrop-blur-md text-[8px] font-bold uppercase tracking-widest text-[#F4ECD8] shadow-md whitespace-nowrap">
            {waitressState === "idle" && "Charlar"}
      ...
      {/* ========================================================
          ...
          ======================================================== */}
      <AnimatePresence>
        {cart.length > 0 && !isOrderOpen && !isWaitressOpen && (
          <motion.div
      ...
      {/* ========================================================
          ...
          ======================================================== */}
      <AnimatePresence>
        {currentSubtitle && !isWaitressOpen && (
          <motion.div
      ...
      {/* ========================================================
          VIRTUAL WAITRESS CHAT DRAWER
          ======================================================== */}
      <VirtualWaitress
        isOpen={isWaitressOpen}
        onClose={() => setIsWaitressOpen(false)}
        history={chatHistory}
        onSendMessage={handleSendMessage}
        waitressState={waitressState}
        setWaitressState={setWaitressState}
        chips={chips}
