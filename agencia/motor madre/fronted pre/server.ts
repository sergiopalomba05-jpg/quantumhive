...
// ----------------------------------------------------------------------------
// API: POST /tts (Text-To-Speech synthesis fallback)
// ----------------------------------------------------------------------------
app.post("/tts", async (req, res) => {
  try {
    ...
    // Expand short unit words so voice reads them cleanly (e.g. 500gr -> 500 gramos)
    text = text
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        speechConfig: {
          voiceConfig: {
            // Zephyr or Kore (Kore is a friendly cheerful female voice)
    ...
    if (base64Audio) {
      ...
      res.setHeader("X-TTS-Provider", "gemini-tts");
      return res.send(buffer);
    }

    // If no voice was produced, let the client know they should use browser TTS fallback
    res.json({ useBrowserTTS: true, text });
  } catch (error: any) {
    console.error("TTS Error:", error);
    // Fall back to client-side speech synthesis if API fails or is not enabled
